...
# --- 10. stylua (Lua フォーマッタ) --------------------------------------
# apt に収録されていないため GitHub Releases からバイナリを取得する。
if has stylua; then
	skip "stylua $(stylua --version)"
else
	log "stylua を ~/.local/bin へ導入"
	tmp="$(mktemp -d)"
	curl -fsSL \
...
link "$DOTFILES_DIR/claude/hooks/bash-guard.sh" "$HOME/.claude/hooks/bash-guard.sh"
